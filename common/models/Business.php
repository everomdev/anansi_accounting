...
class Business extends \yii\db\ActiveRecord
{
    ...
    */
    ...
    $t1 = microtime(true);
    Yii::info(sprintf("getTheoreticalYield: categories loaded count=%d time=%.4fs", count($categories), $t1 - $t0), __METHOD__);
    ...
    $t2 = microtime(true);
    Yii::info(sprintf("getTheoreticalYield: recipes loaded count=%d time=%.4fs", count($recipes), $t2 - $t1), __METHOD__);
    ...
    $t3 = microtime(true);
    Yii::info(sprintf("getTheoreticalYield: combos loaded count=%d time=%.4fs", count($combos), $t3 - $t2), __METHOD__);
    ...
    $t4 = microtime(true);
    Yii::info(sprintf("getTheoreticalYield: grouping done categories=%d recipes=%d combos=%d time=%.4fs", count($categories), count($recipes), count($combos), $t4 - $t3), __METHOD__);
        if ($generalCategory) {
        ...
        $totalCost = $totalItems > 0 ? $totalCostSum / $totalItems : 0;
    $t5 = microtime(true);
    Yii::info(sprintf("getTheoreticalYield: calculations done totalItems=%d time=%.4fs", $totalItems, $t5 - $t4), __METHOD__);

        // Devolver datos basados únicamente en promedios de costos
    $tEnd = microtime(true);
    Yii::info(sprintf("getTheoreticalYield END totalTime=%.4fs", $tEnd - $t0), __METHOD__);
