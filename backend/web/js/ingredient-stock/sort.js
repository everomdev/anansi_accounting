...
document.addEventListener('DOMContentLoaded', function() {
    // Configuración
    const gridId = 'ingredient-stock-grid';
    ...
    function sortTable(columnName, direction) {
        ...
        const getCellValue = (row, index) => {
            ...
            if (columnName === 'avgUnitPrice' || columnName === 'lastUnitPrice' || columnName === 'higherUnitPrice' ) {
                // Para valores de moneda, eliminar símbolos y formateo
                value = parseFloat(value.replace(/[^\d.,]/g, '')
                                       .replace(',', '.')) || 0;
            } else if (columnName === 'recipeCount' || columnName === 'subRecipeCount') {
